...
const addPremiumExpense = async (req, res) => {
  try {
    ...
    const newExpense = await Expense.create({
      amount, description, category, type, note, user_id: userId
    });
...
const getReport = async (req, res) => {
  try {
    ...
    if (period === "daily") {
      ...
      where.created_at = literal("MONTH(created_at) = MONTH(CURDATE()) AND YEAR(created_at)=YEAR(CURDATE())");
    } else if (period === "yearly") {
      where.created_at = literal("YEAR(created_at)=YEAR(CURDATE())");
    } else {
    ...
    const totals = data[0] || { total_income: 0, total_expense: 0 };
...
const downloadReport = async (req, res) => {
  try {
    ...
    const yearly = await Expense.findAll({
      ...
      group: [fn("DATE_FORMAT", col("created_at"), "%M %Y")],
      order: [[fn("MIN", col("created_at")), "ASC"]],
      raw: true
    });

    const notes = await Expense.findAll({
      where: { user_id: userId, note: { [Op.ne]: null } },
    ...
    rows.forEach(r => {
      sheet1.addRow({
        ...
        category: r.category,
        income: r.type === "income" ? Number(r.amount) : null,
        expense: r.type === "expense" ? Number(r.amount) : null,
        savings: null,
        note: r.note || "",
      });
    });
    ...
    sheet1.addRow({});
    sheet1.addRow({ description:"TOTAL", income:totalIncome, expense:totalExpense, savings:totalIncome-totalExpense });
    ...
    yearly.forEach(r=>{
      sheet2.addRow({ month:r.month, income:+r.income, expense:+r.expense, savings:+r.income-+r.expense });
    });

    const yearlyIncome = yearly.reduce((a,r)=>a+Number(r.income),0);
    const yearlyExpense = yearly.reduce((a,r)=>a+Number(r.expense),0);
    sheet2.addRow({});
    sheet2.addRow({ month:"TOTAL", income:yearlyIncome, expense:yearlyExpense, savings:yearlyIncome-yearlyExpense });
    ...
    if (notes.length>0) {
      notes.forEach(n => sheet3.addRow({ date: new Date(n.created_at).toLocaleDateString(), note: n.note }));
    } else {
    ...
    await ExportHistory.create({ user_id:userId, s3_key:fileKey, url });
...
const getExportHistory = async (req, res) => {
  try {
    ...
    const { count: total, rows } = await ExportHistory.findAndCountAll({
      where: { user_id: userId },
      order: [["created_at","DESC"]],
      limit,
      offset,
      attributes: ["id","s3_key","url","created_at"]
    });
